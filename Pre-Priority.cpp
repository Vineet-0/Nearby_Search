...
int main() {
    ...
    while (completed_processes < n) {
        int idx = -1;
        int highest_priority = numeric_limits<int>::min();
        ...
        for (int i = 0; i < n; ++i) {
            if (processes[i].arrival_time <= current_time && processes[i].remaining_time > 0) {
                if (processes[i].priority > highest_priority) {
                    ...
                    idx = i;
                } else if (processes[i].priority == highest_priority && processes[i].arrival_time < processes[idx].arrival_time) {
                    idx = i;
                }
            }
        }
    ...
    for (const auto &p : processes) {
        cout << p.pid << "\t" << p.arrival_time << "\t" << p.burst_time << "\t" << p.priority << "\t" << p.completion_time << "\t" << p.turnaround_time << "\t" << p.waiting_time << endl;
    }
